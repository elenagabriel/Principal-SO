            ...
            if (token != NULL)
            {
                ...
                if (punto != NULL && strcmp(punto, ".asm") == 0)
                {
                    ...
                    else
                    {
                        ...
                        strcpy(nombre_archivo, token); // guardar nombre
                        proceso_id++;
                        strcpy(nombre_archivo_actual, nombre_archivo);

                        fclose(archivo);                 // ya comprobamos, cerramos
                        validar_archivo(nombre_archivo); // mandar a validar
                    }
                }
                else
                {
